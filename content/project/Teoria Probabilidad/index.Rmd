...
library(gganimate)
# Cara = 1, Cruz = 0
lanzamiento <- lapply(1:1000, function(x){sample(c(1,0), size = 1, prob=c(0.5,0.5))})
lanzamiento <- as.data.frame(Reduce(rbind, lanzamiento))
lanzamiento <- lanzamiento %>% mutate(V2=1-V1, index = 1:1000)

graph <- lanzamiento %>% ggplot() + geom_bar(aes(x=V1)) +
    theme_minimal() +
    transition_time(index)
animate(graph, fps = 1, width = 750, height = 450)
